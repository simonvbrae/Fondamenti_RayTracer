...
public:	
	...
	instance* rotate_x(const float r);
	instance* rotate_y(const float r);
	instance* rotate_z(const float r);
...
bool instance::hit(const ray& r, float t_min, float t_max, hit_record& rec, float time) const {
	ray inv_ray;

	inv_ray.o = inverse_matrix * r.o;
	inv_ray.d = inverse_matrix * r.d;

	if (object_ptr->hit(inv_ray, t_min, t_max, rec, time)) {
...
bool instance::hit_shadow(const ray& r, float t_min, float t_max, float time) const {
	ray inv_ray;
	
	inv_ray.o = inverse_matrix * r.o;
	inv_ray.d = inverse_matrix * r.d;

	if (object_ptr->hit_shadow(inv_ray, t_min, t_max, time))
		return (true);
...
instance* instance::translate(const vector3D& trans) {
	translate(trans.x, trans.y, trans.z);
	return this;
}

instance* instance::translate(const float dx, const float dy, const float dz) {
...
instance* instance::scale(const vector3D& s) {
	scale(s.x, s.y, s.z);
	return this;
}

instance* instance::scale(const float a, const float b, const float c) {
...
instance* instance::rotate_x(const float theta) {
...
instance* instance::rotate_y(const float theta) {
...
instance* instance::rotate_z(const float theta) {
	float sin_theta = sin(theta * Deg2Rad);
	float cos_theta = cos(theta * Deg2Rad);
