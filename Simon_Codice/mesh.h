...
bool mesh::hit(const ray& ray, float t_min, float t_max, hit_record &rec, float time) const
{
	...
	for (int i = 0; i < num_faces; i++)
	{
		...
		if (triangle_intersection(ray, t_min, closest_so_far, temp_rec, v0, v1, v2, beta, gamma)) {
			...
			if (areNormals)
			{
				...
				rec.normal = normalize(normals[i0] * alpha + normals[i1] * beta + normals[i2] * gamma);
			}
			...
			point2D texcoord0 = textures[i0];
			point2D texcoord1 = textures[i1];
			point2D texcoord2 = textures[i2];
			
			rec.u = alpha * texcoord0.u + beta * texcoord1.u + gamma * texcoord2.u;
			rec.v = alpha * texcoord0.v + beta * texcoord1.v + gamma * texcoord2.v;

			//rec.texcoord = alpha * texcoord0 + beta * texcoord1 + gamma * texcoord2; prossimamente
		}
	}
	return hit_anything;
}

bool mesh::hit_shadow(const ray& ray, float t_min, float t_max, float time) const
{
	hit_record temp_rec;
	float closest_so_far = t_max;
	float u, v;
